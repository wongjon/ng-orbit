...
void Adafruit_INA219::getCurrentBeginI2C() {
  // Sometimes a sharp load will reset the INA219, which will
  // reset the cal register, meaning CURRENT and POWER will
  // not be available ... avoid this by always setting a cal
  // value even if it's an unfortunate extra step
  ...
  // Now we can safely read the CURRENT register!
  _i2c->beginTransmission(ina219_i2caddr);
  _i2c->write(INA219_REG_CURRENT); // Register
  _i2c->endTransmission();
}

uint16_t Adafruit_INA219::getCurrentRequestI2C() {
  // delay(1); // Max 12-bit conversion time is 586us per sample
  _i2c->requestFrom(ina219_i2caddr, (uint8_t)2);
  // Shift values to create properly formed integer
  uint16_t temp_value = ((_i2c->read() << 8) | _i2c->read());
  temp_value /= ina219_currentDivider_mA;
  return temp_value;
}
